...
| **C.2** 🚧 | Edge de estrategia — backtest de confluencia + walk-forward | 279 tests verdes (+8). Motor de backtest refactorizado para aceptar un `decider` inyectable (comportamiento S3 intacto por defecto). `backtest/confluence.py`: `align_sentiment` (anti look-ahead, merge O(n+m)), `make_confluence_decider` (corre `confluence.decide` + reducción por baja confianza, espejo del Risk Manager), `run_confluence` (A/B sentimiento ON/OFF por la misma ruta), `walk_forward` (robustez por tramos). Runner `backtest/run_confluence.py`. **Hallazgo**: el baseline quant (vía confluencia) pierde de forma robusta en walk-forward (BTC 1h −7%, PF 0.62; consistente en los 4 tramos) → confirma que el quant solo NO tiene edge. **Pendiente de datos**: el brazo "con sentimiento" requiere poblar `sentiment_scores`. |
| **C.3** 🚧 | Fuente de sentimiento $0 (CryptoPanic pasó a pago) | 283 tests verdes (+4). Investigación de mercado: **las noticias cripto históricas gratis desaparecieron** (CryptoPanic free eliminado 1-abr-2026; NewsData/CoinGecko/LunarCrush mueven el archivo histórico a pago $35–300/mo). Única fuente $0 con histórico completo: **índice Fear & Greed** (alternative.me, sin key, desde 2018; incluye componente social Twitter/X ~15%). Construido `sentiment/fear_greed.py` + runner `build_fear_greed.py` (GRATIS, sin claves) → puebla `sentiment_scores` → lo consume `run_confluence` sin cambios. Caveat: es sentimiento de MERCADO (no noticias-evento) y diario; la interpretación momentum vs. contraria es la hipótesis a medir. El cliente CryptoPanic+Claude queda para uso de pago/forward futuro. |
| **C.1** 🚧 | Edge de estrategia — fundación de datos de sentimiento histórico | 271 tests verdes (+12). Hallazgo crítico: el backtest del S3 es quant-puro (PF 0.83) y el sentimiento NUNCA se ha medido en backtest; pero no había corpus histórico (el RSS no da historia). Construido: `sentiment/cryptopanic.py` (ingesta paginada + backoff, mapea a `NewsItem` con el mismo hash de URL), tablas `news`/`sentiment_scores` en storage (acumulativas, idempotentes), `sentiment/scoring.py` (reutiliza filtro+Claude del S4; `analyze_fn` inyectable), runner `build_history.py`. Scores alineados por `published_at` (anti look-ahead). **Operativo (Eduardo, con claves)**: la ingesta real (CryptoPanic free tier + Claude) y su límite de profundidad histórica. **Siguiente (C.2)**: extender el backtester para consumir la serie de sentimiento y correr `confluence.decide` completa + walk-forward, y medir edge quant vs quant+sentimiento. |
| **7.2** ✅ | Hardening de concurrencia y ciclo de vida (pre-testnet) | 259 tests verdes (+12), con **latencia y visibilidad de fills simuladas** en el fake. (1) `asyncio.Lock` serializa la sección crítica del motor. (2) Registro `_in_flight` + ventana de gracia (`reconcile_grace_cycles`): la reconciliación ignora órdenes en vuelo (sin HALT ni RESYNC falsos) y solo detiene ante divergencia *sostenida*. (3) **FLIP desacoplado**: cierra ahora, abre en el ciclo siguiente con snapshot fresco (sin colisión de margen). (4) Backfill REST al (re)arrancar + detección de huecos→rewarm; **adopción** de posiciones del exchange (verificando STOP, halt si desnuda); **persistencia** en SQLite de `peak_wallet`/`day_start_wallet`/kill-switch (sobreviven a reinicios en caliente). `classify_reconciliation` ahora devuelve `ReconReport` (resync vs suspect). `SymbolFilters`/exchange Protocol: `get_open_orders`. |
| **7** 🚧 | Orquestador en vivo + hardening (alertas, reinicio automático). *Dashboard Streamlit: diferido a 7.1* | 247 tests verdes (+24). Núcleo del sprint: `orchestrator/engine.py` une todos los motores en `on_closed_candle` (reconciliación→quant→sentimiento→confluencia→risk→política→acción). Política **una pierna por símbolo** (`policy.py`, pura: abrir/flip/nada). Reconciliación con criterio (`classify_reconciliation`): resync benigno por SL/TP vs **halt** peligroso (circuit breaker c). Hardening: `AlertSink` (log/grabador, enchufable a webhook) en kill switch/halt/feed/fallos; supervisión de tareas con auto-restart+backoff; watchdog del feed (circuit breaker a). Punto de entrada `main.py --live` (datos spot mainnet + órdenes futuros testnet). **Operativo (red, lo corre Eduardo)**: la semana de paper trading en testnet y el wiring del poller de sentimiento en vivo. **Diferido a 7.1**: dashboard Streamlit de monitoreo (capa de solo-lectura sobre el storage). Revisión de métricas antes de discutir capital real. |
